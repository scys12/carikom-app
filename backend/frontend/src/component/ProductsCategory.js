                        ...
                        <Row className="show-grid text-center">
                            {this.state.items.length > 0  && this.state.items.map(item =>
                                <div key={item.id} className="item-wrapper">
                                    <Card>
                                        <div className="text-center"><a href={`/user/profile/${item.user.username}`} className="seller-section"><FontAwesomeIcon icon={faUserTag} size="1x"/> {item.user.username} </a><Card.Img variant="top" src={`/images/${item.category.name}.png`} className="item-img" /></div>
                                        <Card.Body>
